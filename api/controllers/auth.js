...
export const login = (req, res) => {
    ...
    db.query(q, [req?.body.username], (err, data) => {
        ...
        if (data.length === 0) return res?.status(404).json("Такого пользователя нет");

        // Check password
        console.log(`${req.body.password} ?= ${data[0].password} : ${req.body.password === data[0].password}`)
        const isPasswordCorrect = bcrypt.compareSync(req.body.password, data[0].password);

        if (!isPasswordCorrect) return res.status(400).json("Неверное имя пользователя и/или пароль!");
